...
const Product = ({ product, setModalProudcts}) => {
    return (
        <div className="card bg-base-100 shadow-2xl py-7 rounded-2xl border-2 border-black mt-5">
            ...
            <div onClick={()=>setModalProudcts(product)} className="w-full text-center">
                <button className="btn btn-neutral w-80 mx-auto" onClick={()=>window.my_modal_3.showModal()}>Add to cart</button>
            </div>
        </div>
    );
};
